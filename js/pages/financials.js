...
Router.register('financials', async (container) => {
    ...
    function renderKPIs() {
        ...
        const totalRevenue = salesData.reduce((sum, s) => sum + (parseFloat(s.total_contract_value) || 0), 0);
    ...
    function renderRevenueChart() {
        ...
        salesData.forEach(s => {
            ...
            if (month) revenueByMonth[month] = (revenueByMonth[month] || 0) + (parseFloat(s.total_contract_value) || 0);
        });
        ...
        if (currentTab === 'sales') {
            DataTable.render(tableEl, {
                columns: [
                    ...
                    { key: 'total_contract_value', label: 'TCV', render: (v) => Utils.formatCurrency(v) },
                    { key: 'payment_status', label: 'Payment', render: (v) => Utils.statusBadge(v) },
                    { key: 'contract_start_date', label: 'Start', render: (v) => Utils.formatDate(v) }
                ],
                data: salesData,
                searchable: true,
                pageSize: 15,
